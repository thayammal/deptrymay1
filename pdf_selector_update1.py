...
def download_pdf_and_save_folder(pdf_link,folder,cat_name,cat_val):
    ...
    if cat_name != 0:
        file = open(f"{folder}/{cat_name}/{cat_val}_{time}__{pdf_name}", "wb")
    else:
        file = open(f"{folder}/{cat_val}_{time}__{pdf_name}", "wb")
...
def pdf_score(cat_name,sorted_word_count_in_pdf):
    df = pd.read_excel("D:\\Infoware\\Cyber_WebScrap_Oct\\May2023\\webscrap_try\\utils\\Regulator.xlsx", sheet_name='scoring')
    if cat_name == 'policy':
    ...
    try:
        ...
        if ispdfexit:
            ...
            with BytesIO(my_raw_data) as data:
                ...
                pages_text1 = read_pdf.getPage(0).extractText()
                pages_text2 = read_pdf.getPage(1).extractText() 
                # print("pages text >>>>>>>>>. ",pages_text)
                for page in range(read_pdf.getNumPages()):
                    each_page_content = read_pdf.getPage(page).extractText()
...
#pdf_link = 'https://www.gov.il/BlobFolder/news/international_strategy/en/Israel%20International%20Cyber%20Strategy.pdf'

#folder = '/home/oem/python-webscrap-policy-main/israel/CyberSecurity'
#pdf_selector(pdf_link,folder,'israel')
